...
class RecordAudioWindow:
    ...
    def process_audio(self, audio_data, patient_name, patient_id):
        ...
        if audio_data:
            ...
            generator = EHRGenerator(model_path, tokenizer_path)
            generated_text = generator.generate_text(transcribed_text, patient_name, patient_id)
            if generated_text:
                st.write("Generated EHR:", generated_text)
    ...
    def transcribe_audio(self, audio_data):
        ...
        try:
            ...
            with open(temp_audio_file_path, "rb") as audio_file:
                ...
                transcribed_text = output_json["results"]["channels"][0]["alternatives"][0]["transcript"]
...
class EHRGenerator:
    ...
    def generate_text(self, transcribed_text, patient_name, patient_id):
        if not transcribed_text or not patient_name or not patient_id:
            st.error("Please provide transcribed text, patient name, and patient ID.")
            return None
        ...
        try:
            ...
            inputs = self.tokenizer.encode(query + transcribed_text, return_tensors="pt", max_length=1024, truncation=True)
            outputs = self.model.generate(inputs, max_length=1000, temperature=0.7, num_return_sequences=1)
